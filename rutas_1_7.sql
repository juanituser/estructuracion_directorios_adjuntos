-- Fijar esquemas
set search_path to 	
	ladm, -- Esquema del modelo de captura en campo
	public;
...
adjuntos_interesados as (
    ...
    from cca_predio cp
    left join cca_derechointeresadofuente cd on cd.predio = cp.t_id
    left join cca_adjuntointeresadovalor cai on cai.cca_derechointersdfnte_i_adjunto = cd.t_id
...
adjuntos_fuente as (
    select 
        ...
        case 
		WHEN cd.fa_tipo in (select t_id FROM cca_fuenteadministrativatipo where ilicode like '%Acto_Administrativo%')
    	THEN '_AD'
    	WHEN cd.fa_tipo in (select t_id FROM cca_fuenteadministrativatipo where ilicode like '%Escritura_Publica%')
    	THEN '_EP'
    	WHEN cd.fa_tipo in (select t_id FROM cca_fuenteadministrativatipo where ilicode like '%Sentencia_Judicial%')
    	THEN '_SJ'
    	WHEN cd.fa_tipo in (select t_id FROM cca_fuenteadministrativatipo where ilicode like '%Documento_Privado%')
    	THEN '_DP'
    	else concat('_',(select ilicode FROM cca_fuenteadministrativatipo where cd.fa_tipo=t_id))
	end as sufijo,
	ROW_NUMBER () OVER (
	PARTITION BY cp.numero_predial
    ORDER BY
    ...
    from cca_predio cp
    left join cca_derechointeresadofuente cd on cd.predio = cp.t_id
    inner join cca_adjuntofuenteadministrativavalor caf on caf.cca_derechointersdfnte_fa_adjunto = cd.t_id
...
COPY rutas TO 'C:/Users/PC/Documents/CEICOL/adjuntos/valle_guamuez_1_7/rutas.csv' WITH (FORMAT CSV, HEADER);
